...
public class UpdateOrderStatusCommandValidator implements CommandValidator {
    ...
    public boolean validate(HttpServletRequest request, HttpServletResponse response) {
        ...
        return (CommandValidatorHelper.getInstance().isNullValidate(new String[]{ORDER_STATUS_ATTRIBUTE},
                RESULT_ATTRIBUTE, ADMIN_ORDER_DESTINATION_PAGE, message, request, response))
                && CommandValidatorHelper.getInstance().isEmptyValidate(new String[]{ORDER_STATUS_ATTRIBUTE},
                RESULT_ATTRIBUTE, ADMIN_ORDER_DESTINATION_PAGE, message, request, response);
    }
}
